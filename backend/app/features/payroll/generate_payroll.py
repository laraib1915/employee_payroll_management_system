...
from app.features.payroll.calculate_absences import (
    calculate_absences
)

from app.features.payroll.calculate_lates import (
    calculate_lates
)

from app.features.payroll.calculate_halfdays import (
    calculate_halfdays
)

from app.features.payroll.calculate_salary import (
    calculate_salary
)
...
async def generate_payroll(
    ...
    try:
        ...
        for attendance_doc in attendance_documents:
            ...
            employee_no = str(
                attendance_doc["employee_no"]
            )

            # ===============================
            # Prevent Duplicate Payroll
            # ===============================
            existing_payroll = await db[
                collections.PAYROLL
            ].find_one(
                {
            ...
            # Daily Salary
            # ===============================
            monthly_salary = employee[
                "monthly_salary"
            ]

            working_days = settings[
            ...
            for override in (
                payload.
                annual_leave_overrides
            ):

                if (
                    override.employee_no
                    == employee_no
                ):

                    annual_leave_override = (
                        override.
                        annual_leaves_to_apply
                    )
