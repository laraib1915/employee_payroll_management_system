    ...
    try:
        ...
        worksheet.append(
            [
                ...
                payroll_summary.get("total_employees_processed")
            ]
        )
        worksheet.append(
            [
                "Total Gross Salary",
                payroll_summary.get("total_gross_salary")
            ]
        )
        worksheet.append(
            [
                "Total Deductions",
                payroll_summary.get("total_deductions")
            ]
        )
        worksheet.append(
            [
                "Total Net Payroll",
                payroll_summary.get("total_net_payroll")
            ]
        )
        worksheet.append(
            [
                ...
                payroll_summary.get("total_amount_required_for_salary_disbursement")
            ]
        )
        # Payroll Summary Styling
        summary_start_row = (worksheet.max_row - 6)
        for row in range(
            summary_start_row,
            worksheet.max_row + 1
        ):
        ...
        logger.exception(f"Error exporting salary sheet: "f"{str(e)}")
        raise